...
const FORFAIT_VALUES = ["HEBDO", "MENSUEL", "TRIMESTRIEL", "ANNUEL"];
...
exports.SubscriptionService = {
    async create(input) {
        ...
        const subscriptionRow = await models_1.sequelize.transaction(async (t) => {
            const member = await models_1.Member.findByPk(idMembre, {
                transaction: t,
                lock: t.LOCK.UPDATE,
            });
            if (!member) {
            ...
            const activity = await models_1.Activity.findByPk(idActivity, {
                transaction: t,
                lock: t.LOCK.UPDATE,
            });
            if (!activity) {
            ...
            if (input.montant_override != null && input.montant_override !== undefined) {
                ...
                const prixForfait = typeof raw === "number" ? raw : parseFloat(String(raw ?? "0")) || 0;
    ...
    async list(filters) {
        ...
        if (filters.activityId != null)
            where.id_activity = filters.activityId;
        if (filters.status === "active") {
            where.date_prochain_paiement = { [sequelize_1.Op.gte]: todayYmd() };
        }
    ...
    async expiringSoon(days) {
        const start = todayYmd();
        const end = addDaysYmd(start, days);
        return models_1.Subscription.findAll({
            where: {
                date_prochain_paiement: {
                    [sequelize_1.Op.and]: [{ [sequelize_1.Op.gte]: start }, { [sequelize_1.Op.lte]: end }],
                },
            },
            include: [
                { model: models_1.Member, as: 'member', required: false },
    ...
    async remove(id) {
        const [n] = await models_1.Subscription.update({ active: false }, { where: { id } });
        if (n === 0)
            throw Object.assign(new Error("Subscription not found"), { status: 404 });
    },
};
